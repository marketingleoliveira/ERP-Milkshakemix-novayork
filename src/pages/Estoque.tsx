...
export default function Estoque() {
  ...
  const addMovement = useMutation({
    mutationFn: async (m: { product_id: string; type: "entrada" | "saida" | "ajuste"; quantity: number; reason: string }) => {
      const { error: movErr } = await supabase.from("stock_movements").insert({ ...m, created_by: user?.id });
      if (movErr) throw movErr;

      const product = products?.find((p) => p.id === m.product_id);
      if (!product) return;

      const newQty = m.type === "entrada"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={showAddProduct} onOpenChange={setShowAddProduct}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="font-heading">Adicionar Produto</DialogTitle>
            </DialogHeader>
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        {isLow ? (
                          ...
                          <Badge variant="secondary" className="text-xs bg-success/10 text-success border-0">OK</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        <Dialog open={showMovement === p.id} onOpenChange={(o) => setShowMovement(o ? p.id : null)}>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              <DialogTitle className="font-heading">Movimentar: {p.name}</DialogTitle>
                            </DialogHeader>
...
function AddProductForm({ categories, onSubmit, loading }: {
...
function MovementForm({ productId, onSubmit, loading }: {
  productId: string;
  onSubmit: (m: { product_id: string; type: "entrada" | "saida" | "ajuste"; quantity: number; reason: string }) => void;
  loading: boolean;
}) {
